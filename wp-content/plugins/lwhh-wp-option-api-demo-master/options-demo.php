...
add_action( 'wp_ajax_options_display_result', function () {
    ...
    if ( wp_verify_nonce( $_POST['nonce'], 'options_display_result' ) ) {
        ...
        elseif ( 'export-option' == $task ) {
            $key_normal = ['od-country','od-capital'];
            $key_array = ['od-array','od-new-array'];
            $key_json = ['od-array-json'];
...
add_filter( 'option_od-country', function ( $value ) { // option ar filter likhar jonno optin underscore tarpor key ar name hobe
